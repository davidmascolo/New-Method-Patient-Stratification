...
df_clinical <- read.csv(paste0(data_path, "LUNG_Clinical.csv"))
id_patients <- read.csv(paste0(data_path, "id_Lung_patients.csv"))
...
df_clinical_subset <- df_clinical %>% 
  select(synchronous_malignancy, ajcc_pathologic_stage,
         ...
         ajcc_pathologic_n, ajcc_pathologic_m, classification_of_tumor,
...
## Formatting
df_clinical_subset$ajcc_pathologic_stage <- fct_infreq(df_clinical_subset$ajcc_pathologic_stage)
df_clinical_subset$ajcc_pathologic_stage <- factor(df_clinical_subset$ajcc_pathologic_stage,
                                                   levels = c("Stage I", "Stage IA", "Stage IIA",
                                                              "Stage IIIA", "Stage IB", "Stage IIB",
...
(p7 <- ggplot(df_clinical_subset,
    ...
    geom_text(aes(label = paste0(round(after_stat(count)/sum(after_stat(count)) * 100, 2), "%)")),
              stat = "count", vjust = -0.3, hjust = 0.3, size = 4) +
    scale_fill_manual(values = c("Lower lobe, lung" = "#DEEBF7",
                                 "Lung, NOS" = "#C6DBEF",
                                 "Main bronchus" = "#9ECAE1",
                                 "Middle lobe, lung" = "#6BAED6",
                                 "Upper lobe, lung" = "#4292C6"),
                      name = "Origin Point") +
...
(p8 <- ggplot(df_clinical_subset,
    ...
    geom_text(aes(label = paste0(after_stat(count), ", ", "(",
                                 round(after_stat(count)/sum(after_stat(count)) * 100, 2), "%)")),
              stat = "count", vjust = -0.3, hjust = 0.2, size = 4)  +
    scale_fill_manual(values = c("Basaloid squamous cell carcinoma" = "#2171B5",
                                 "Squamous cell carcinoma, NOS" = "#08306B"),
                      name = "Diagnosis") +

...
df_clinical <- read.csv(paste0(data_path, "COLON_Clinical.csv"))
id_patients <- read.csv(paste0(data_path, "id_COLON_patients.csv"))
...
(p7 <- ggplot(df_clinical_subset,
    ...
    geom_text(aes(label = paste0(round(after_stat(count)/sum(after_stat(count)) * 100, 2), "%")),
              stat = "count", vjust = -0.3, hjust = 0.3, size = 4) +
    scale_fill_manual(values = c("Ascending colon"       = "#DEEBF7",
                                 "Cecum"                 = "#C6DBEF",
                                 "Colon, NOS"            = "#9ECAE1",
                                 "Descending colon"      = "#6BAED6",
                                 "Rectosigmoid junction" = "#4292C6",
                                 "Sigmoid colon"         = "#2171B5",
                                 "Transverse colon"      = "#08519C"),
                      name = "Origin Point") +
...
(p8 <- ggplot(df_clinical_subset,
    ...
    scale_fill_manual(values = c("Adenocarcinoma with neuroendocrine differentiations" = "#DEEBF7",
                                 "Adenocarcinoma, NOS"                                 = "#C6DBEF",
                                 "Adenosquamous carcinoma"                             = "#9ECAE1",
                                 "Carcinoma, NOS"                                      = "#6BAED6",
                                 "Mucinous adenocarcinoma"                             = "#4292C6"),
                      name = "Diagnosis") +

...
GBM_Survival$PatientIDModified <- gsub("TCGA-([0-9]{2})-([0-9]{4}).*",
                                       "\\1.\\2", GBM_Survival$PatientID)
id <- GBM_Survival
## Extract data
...
p1 <- ggsurvplot(fit_survival, data = d, xlim = c(0, 125),
           ggtheme = theme_bw(), palette = c("green", "blue", "red"),
           legend.title = "Subtype",
           legend.labs = c("01", "02", "03"),
           xlab = "Survival Time (Months)",
           ylab = "Survival Probability",
           title = "GBM Survival Analysis - Algorithm Name",
...
patient_id_type_01 <- sub("^(TCGA-[0-9]+)-([0-9]+).*", "\\1-\\2",
                          patient_id_type_01)
patient_id_type_02 <- sub("^(TCGA-[0-9]+)-([0-9]+).*", "\\1-\\2",
                          patient_id_type_02)  
patient_id_type_03 <- sub("^(TCGA-[0-9]+)-([0-9]+).*", "\\1-\\2",
                          patient_id_type_03)  
